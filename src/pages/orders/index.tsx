...
export function OrderListPage(props: OrderListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getOrders({
        relations: [
          'user_order_restaurant_manager_idTouser',
          'user_order_delivery_driver_idTouser',
          'user_order_customer_service_rep_idTouser',
        ],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_order_restaurant_manager_idTouser',
          header: 'User Order Restaurant Manager Id Touser',
          accessorKey: 'user_order_restaurant_manager_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_order_restaurant_manager_idTouser?.id}`}
            >
              {record.user_order_restaurant_manager_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_order_delivery_driver_idTouser',
          header: 'User Order Delivery Driver Id Touser',
          accessorKey: 'user_order_delivery_driver_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_order_delivery_driver_idTouser?.id}`}
            >
              {record.user_order_delivery_driver_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_order_customer_service_rep_idTouser',
          header: 'User Order Customer Service Rep Id Touser',
          accessorKey: 'user_order_customer_service_rep_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_order_customer_service_rep_idTouser?.id}`}
            >
              {record.user_order_customer_service_rep_idTouser?.email}
            </Link>
          ),
        }
      : null,
